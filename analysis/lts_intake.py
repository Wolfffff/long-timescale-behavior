...
exp1_cam3_h5s = [
    "exp2_cam3_0through301.tracked.analysis.h5"
]
exp1_cam3_h5s = [path_name + "/" + filename for filename in exp1_cam3_h5s]
...
expmt_dict = {
    "exp1_cam1": {
        ...
        "video": "exp1_cam1.mkv",
        "frame_rate": 100,
        "start_time": datetime.strptime("0-21:33:00", FMT),
        "camera": "1",
        "experiment": "1",
        ...
        "h5s": exp1_cam3_h5s,
        "frame_rate": 100,
        "start_time": datetime.strptime("0-21:27:00", FMT),
        "camera": "3",
        "experiment": "1",
        "video_path": "test"
    }
...
for expmt_name in list(expmt_dict.keys()):
    try:
        ...
        logger.error(f"Failed to load keypoints for {expmt_name}")
...
for key in list(expmt_dict.keys()):
    ...
    logger.info(f"Final assignment: {freq}")
...
# %%
# python ../../sleap/sleap/nn/tracking.py ${FILE} --tracker simple --similarity iou --match greedy --track_window 5 --post_connect_single_breaks 1 --target_instance_count 4 --clean_instance_count 4 --output ${FILE%.*}.tracked.slp &> ${FILE%.*}_tracking.log &